...
class MLPWithAttention(torch.nn.Module):
    def __init__(self, *sizes, batchnorm=False, dropout=False, num_heads=1):
        super().__init__()
        self.attention = torch.nn.MultiheadAttention(embed_dim=sizes[0], num_heads=num_heads)
        fcs = []
        for i in range(1, len(sizes)):
            fcs.append(torch.nn.Linear(sizes[i - 1], sizes[i]))
            if i < len(sizes) - 1:
...
class SignalProp(torch.nn.Module):
    def __init__(self, in_nf, in_cell_num_luts, in_cell_lut_sz, out_nf, out_cef, h1=32, h2=32, lut_dup=4):
        ...
        self.MLP_cellarc_msg = MLPWithAttention(self.out_nf + 2 * self.in_nf + self.in_cell_num_luts * self.lut_dup, 64,
                                                64, 64, 1 + self.h1 + self.h2 + self.out_cef)
...
if __name__ == '__main__':
    model = TimingGCN()
    model.cuda()
    from data_graph import data_train
    for k, (g, ts) in data_train.items():
        y = model(g, ts, groundtruth=False)
        pass